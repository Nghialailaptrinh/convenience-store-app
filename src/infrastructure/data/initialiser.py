...
from infrastructure.data.models import Base, ProductRecord
        ...
        for number, name, description, amount, category in products:
            ...
            if session.get(ProductRecord, product_id) is None:
                session.add(ProductRecord(
                    id=product_id, name=name, description=description, amount=amount,
                    currency="VND", category_id=str(UUID(int=100 + category)), is_active=True,
                ))
